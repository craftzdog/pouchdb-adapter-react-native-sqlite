  ...
  describe('suite2 test.attachments.js-' + adapter, function () {
    ...
    var pngAttDoc = {
      ...
      _attachments: {
        'foo.png': {
          ...
          data:
            'iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAMAAAAoLQ9TAAAAMFBMVEX+9+' +
    ...
    it('#2858 {binary: true} in allDocs() 5', function () {
      ...
      return db.bulkDocs(docs).then(function () {
        return db
          .allDocs({
            keys: [binAttDoc._id, binAttDoc2._id, pngAttDoc._id, 'foo', 'bar'],
            attachments: true,
            binary: true,
            include_docs: true
          })
    ...
    it('#3881 filter extraneous keys from _attachments', function () {
      ...
      return db
        ...
        .then(function (doc) {
          var keys = Object.keys(doc._attachments['foo.txt'])
            .filter(function (x) {
    ...
    it('Test getAttachment for _local doc - should not return attachment', async () => {
      ...
      if (adapter === 'sqlite3') {
        ...
        if (serverType === 'couchdb') {
          ...
          body.reason.should.equal(
            '_local documents do not accept attachments.'
          )
        } else if (
          serverType === 'pouchdb-express-router' ||
          serverType === 'express-pouchdb'
        ) {
    ...
    it('Test attachments:true for _local doc', async () => {
      ...
      if (adapter === 'sqlite3') {
        ...
        if (serverType === 'couchdb') {
          ...
          doc._attachments['foo.txt'].content_type.should.equal('text/plain')
          JSON.parse(
            decodeBase64(doc._attachments['foo.txt'].data)
          ).should.deep.equal({
            error: 'not_found',
            reason: 'missing'
          })
        ...
        db.allDocs({ include_docs: true }, function (err, res) {
          for (var i = 0; i < docs.length; i++) {
            ...
            for (var j = 0; j < attachmentsNb; j++) {
              res.rows[i].doc._attachments['att' + j].stub.should.equal(
                true,
          ...
          db.changes({
            ...
            .on('complete', function (res) {
              ...
              for (var i = 0; i < 3; i++) {
                attachmentsNb =
                  typeof docs[i]._attachments !== 'undefined'
                    ...
                    : 0
                for (var j = 0; j < attachmentsNb; j++) {
                  res.results[i].doc._attachments['att' + j].stub.should.equal(
                    true,
              ...
              should.not.exist(
                res.results[0].doc._attachments,
    ...
    it('Test putAttachment with base64 plaintext', function () {
      var db = new PouchDB(dbs.name)
      return db
        .putAttachment('doc', 'att', null, 'Zm9v', 'text/plain')
        .then(function () {
    ...
    it('Test putAttachment with invalid base64', function () {
      var db = new PouchDB(dbs.name)
      return db
        .putAttachment('doc', 'att', null, '\u65e5\u672c\u8a9e', 'text/plain')
    ...
    it('Test getAttachment with empty text', function (done) {
      ...
      db.put(binAttDoc2, function (err) {
        ...
        db.getAttachment('bin_doc2', 'foo.txt', function (err, res) {
          ...
          ;(typeof res).should.equal(
            'object',
            'res is object, ' + 'not a string'
          )
    ...
    it('Test getAttachment with PNG', function (done) {
      ...
      db.put(pngAttDoc, function (err) {
        ...
        db.getAttachment('png_doc', 'foo.png', function (err, res) {
          ...
          testUtils.base64Blob(res, function (data) {
            data.should.equal(
              pngAttDoc._attachments['foo.png'].data,
              'correct data'
            )
            done()
          })
        ...
        .on('change', function (change) {
          if (change.id === 'anotherdoc2') {
            ...
            db.get(change.id, { attachments: true }, function (err, doc) {
              doc._attachments.should.be.an(
                'object',
                'doc has attachments object'
              )
              should.exist(
                doc._attachments.mytext,
    ...
    it('Test a document with a json string attachment', function (done) {
      ...
      db.put(jsonDoc, function (err, results) {
        ...
        db.get(results.id, function (err, doc) {
          ...
          doc._attachments['foo.json'].content_type.should.equal(
            'application/json',
          ...
          db.getAttachment(results.id, 'foo.json', function (err, attachment) {
            ...
            testUtils.readBlob(attachment, function () {
              jsonDoc._attachments['foo.json'].data.should.equal(
                'eyJIZWxsbyI6IndvcmxkIn0=',
                'correct data'
              )
              done()
            })
    ...
    it('Test synchronous putAttachment with no text data', function (done) {
      ...
      db.putAttachment('a', 'foo2.txt', '', '', 'text/plain', function (err) {
        ...
        db.get('a', { attachments: true }, function (err, doc) {
          ...
          doc._attachments['foo2.txt'].content_type
            .indexOf('text/plain')
            ...
            testCases.map(function (testCase) {
              ...
              return promise
                ...
                .then(function (bin) {
                  bin.should.equal(
                    expected,
                    "didn't get blob we expected for rev"
                  )
                })
            })
          )
    ...
    it('putAttachment in new doc with base64', function () {
      ...
      return db
        .putAttachment('foo', 'att', 'Zm9v', 'text/plain')
        .then(function () {
          return db.get('foo', { attachments: true })
        })
        .then(function (doc) {
    ...
    it('#2818 - save same attachment in different revs', function () {
      ...
      return db
        ...
        .then(function (doc) {
          doc._attachments['att'].content_type.should.match(/^text\/plain/)
          doc._attachments['att'].data.length.should.be.above(
            0,
            'attachment exists'
          )
        })
    ...
    it('#3008 test correct encoding/decoding of \\u0000 etc.', function () {
      ...
      var db = new PouchDB(dbs.name)
      return db
        .putAttachment('foo', 'foo.bin', base64, 'image/png')
        .then(function () {
      ...
      it('putAttachment and getAttachment with big png data', function (done) {
        function getData(cb) {
          if (typeof process !== 'undefined' && !process.browser) {
            ...
            // browser
            testUtils
              .asyncLoadScript('deps/bigimage.js')
...
repl_adapters.forEach(function (adapters) {
  describe(
    'suite2 test.attachments.js- ' + adapters[0] + ':' + adapters[1],
    function () {
      var dbs = {}
      ...
      it('Attachment types replicate', function () {
        ...
        return remote
          ...
          .then(function (bin) {
            bin.should.equal(
              testUtils.atob('VGhpcyBpcyBhIGJhc2U2NCBlbmNvZGVkIHRleHQ=')
            )
          })
      })
