describe('test.basics.js-sqlite3', function () {
  var dbs = {}
  ...
  it('[4595] should emit error event on xhr error', function (done) {
    var invalidUrl = 'http:///'
    new PouchDB(dbs.name).replicate.to(invalidUrl, {}).on('error', function () {
      done()
    })
  })
  ...
  it('Remove doc twice with specified id', function () {
    var db = new PouchDB(dbs.name)
    return db
      .put({ _id: 'specifiedId', test: 'somestuff' })
      .then(function () {
  ...
  it('Doc validation', function (done) {
    ...
    db.bulkDocs({ docs: bad_docs }, function (err) {
      ...
      err.message.should.equal(
        testUtils.errors.DOC_VALIDATION.message + ': _zing',
  ...
  it('Put doc without _id should fail', function (done) {
    ...
    db.put({ test: 'somestuff' }, function (err) {
      should.exist(err)
      err.message.should.equal(
        testUtils.errors.MISSING_ID.message,
        'correct error message returned'
      )
      done()
    })
  ...
  it('Put doc with bad reserved id should fail', function (done) {
    ...
    db.put(
      ...
      function (err) {
        ...
        err.message.should.equal(
          testUtils.errors.RESERVED_ID.message,
          'correct error message returned'
        )
        done()
      }
  ...
  it('#4126 should not store raw Dates', function () {
    ...
    return db
      ...
      .then(function (res) {
        ...
        origDocs[2].deep.deeper.deeperstill.should.be.instanceof(
          Date,
          'date not modified'
        )
      })
  ...
  ;[
    ...
    describe(`Should error when document is not an object #${idx}`, () => {
      ...
      it(
        ...
        expectNotAnObject(() => db.post(badDoc))
      )
      it(
        'should error for .put()',
        expectNotAnObject(() => db.put(badDoc))
      )
      it(
        'should error for .bulkDocs()',
        expectNotAnObject(() => db.bulkDocs({ docs: [badDoc] }))
      )
    })
  })
  ...
  it('Test rev purge', function () {
    ...
    return db
      ...
      .then(function () {
        should.fail('doc should not exist')
      })
      .catch(function (err) {
        if (!err.status) {
          throw err
    ...
    return db1.info().then(function () {
      ...
      return db2.info().then(function () {
        if (typeof db1._blobSupport !== 'undefined') {
          db1._blobSupport.should.equal(db2._blobSupport, 'same blob support')
        } else {
