...
commonUtils.adapters = function () {
  var adapters = commonUtils.isNode()
...
commonUtils.plugins = function () {
  var plugins = commonUtils.isNode()
    ? process.env.PLUGINS
...
commonUtils.loadPouchDBForNode = function () {
  // Neutralized for React Native: the conformance suite uses the globally
  // configured PouchDB from tests/setup.js (op-sqlite adapter), never this
  // node/CLI loader. Its original body did dynamic `require(variable)` /
  // `require(`${tpl}`)` calls, which Metro's static bundler cannot transform.
  throw new Error('loadPouchDBForNode is not supported in React Native')
}
