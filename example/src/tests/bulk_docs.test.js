  ...
  describe('test.bulk_docs.js-' + adapter, function () {
        ...
        .then(function () {
          return db.bulkDocs({ docs }).then(function (results) {
            results[0].name.should.equal(
              'conflict',
              'First doc should be in conflict'
            )
    ...
    it('Test errors on invalid doc id', function (done) {
      ...
      db.bulkDocs({ docs }, function (err, info) {
        err.status.should.equal(
          testUtils.errors.RESERVED_ID.status,
    ...
    it('No docs', function (done) {
      ...
      db.bulkDocs({ doc: [{ foo: 'bar' }] }, function (err) {
        err.status.should.equal(
          testUtils.errors.MISSING_BULK_DOCS.status,
          'correct error returned'
        )
        err.message.should.equal(
          testUtils.errors.MISSING_BULK_DOCS.message,
      ...
      db.bulkDocs({ docs }, { new_edits: false }, function () {
        db.get('foo', { open_revs: 'all' }, function (err, res) {
          res.sort(function (a, b) {
            return a.ok._rev < b.ok._rev ? -1 : a.ok._rev > b.ok._rev ? 1 : 0
          })
    ...
    it('Testing successive new_edits to the same doc, different content', function (done) {
      var db = new PouchDB(dbs.name)
      var docsA = [
        {
          _id: 'foo321',
    ...
    it('Bulk docs not an object', function (done) {
      ...
      db.bulkDocs({ docs: ['foo'] }, function (err) {
        should.exist(err, 'error reported')
        err.status.should.equal(
          testUtils.errors.NOT_AN_OBJECT.status,
          'correct error status returned'
        )
        err.message.should.equal(
          testUtils.errors.NOT_AN_OBJECT.message,
          'correct error message returned'
        )
      })
      db.bulkDocs({ docs: [[]] }, function (err) {
    ...
    it('Bulk docs two different revisions to same document id', function () {
      ...
      return db
        .bulkDocs([a_doc, b_doc], { new_edits: false })
        .then(function () {
        ...
        .then(function (resp) {
          ...
          ;(
            ...
            (resp[0].ok._rev === b_doc._rev && resp[1].ok._rev === a_doc._rev)
          ).should.equal(true)
        })
    })
    ...
    it('2839 implement revs_limit', function (done) {
      ...
      db.bulkDocs([doc1], { new_edits: false })
        .then(function () {
          return db.get('doc', { revs: true })
        })
        .then(function (doc) {
    ...
    it('5793 bulk docs accepts _conflicts when new_edits=false', function () {
      ...
      return db
        .bulkDocs({ docs: [newdoc] }, { new_edits: false })
        .then(function () {
          return db.allDocs()
        })
