// RN note: 'local' and 'sqlite3' both resolve to react-native-sqlite here (only
// 'http' hits the remote pouchdb-server via testUtils.couchHost). For local-only
// replication, use [['sqlite3','sqlite3']] (no server needed).
var adapters = [
  ['sqlite3', 'http'], // push: local sqlite -> remote pouchdb-server
  ['http', 'sqlite3'] // pull: remote pouchdb-server -> local sqlite
]

adapters.forEach(function (adapters) {
  describe(
    'suite2 test.replication.js-' + adapters[0] + '-' + adapters[1],
    function () {
      var dbs = {}
      ...
      it('Test basic pull replication plain api 2', function (done) {
        var remote = new PouchDB(dbs.remote)
        remote.bulkDocs({ docs }, {}, function () {
          PouchDB.replicate(dbs.remote, dbs.name).on(
            'complete',
            function (result) {
              result.ok.should.equal(true)
              result.docs_written.should.equal(docs.length)
      ...
      it('Test live push checkpoint', function (done) {
        ...
        function isFinished() {
          ...
          db.replicate.to(dbs.remote).on('error', done).on('complete', complete)
        }
          ...
          .then(function () {
            PouchDB.replicate(db, remote)
              ...
              .on('complete', function (result) {
                result.docs_read.should.equal(1)
                result.docs_written.should.equal(1)
                db.bulkDocs({ docs: docs.slice(1, 2) }).then(function () {
                  expectedSince = 1
                  PouchDB.replicate(db, remote)
                    .on('error', done)
                    .on('complete', function (result) {
      ...
      it('#3136 winningRev has a lower seq', function () {
        ...
        var tree = [
          [
            { _id: 'foo', _rev: '1-a', _revisions: { start: 1, ids: ['a'] } },
            {
              _id: 'foo',
              _rev: '2-e',
              _deleted: true,
        ...
        return db
          ...
          .then(function () {
            ...
            return remote.put(rev4Doc).then(function () {
              return PouchDB.replicate(remote, db)
                .then(function () {
                  return PouchDB.replicate(db, remote)
                })
                .then(function () {
                ...
                .then(function (localChanges) {
                  return remote
                    ...
                    .then(function (remoteChanges) {
                      ...
                      localChanges.should.deep.equal(
                        remoteChanges,
                        'same changes'
                      )
                    })
                                ...
                                db1.get('foo', function (err, doc) {
                                  ...
                                  db2.get('foo', function (err, doc) {
                                    ...
                                    db1.allDocs(
                                      { include_docs: true },
                                      function (err, res) {
                                        res.rows.should.have.length.above(
                                          0,
                                          'first'
                                        )
                                        // redundant but we want to test it
                                        res.rows[0].doc.value.should.equal(
                                          'db1'
                                        )
                                        db2.allDocs(
                                          { include_docs: true },
      ...
      it('test-cancel-pull-replication', function (done) {
        ...
        remote.bulkDocs(docs).then(function () {
          ...
          var replicate = db.replicate
            .from(remote, { live: true })
      ...
      it('Test basic events', function (done) {
        ...
        db.bulkDocs({ docs }).then(function () {
          db.replicate.to(dbs.remote).on('complete', function (res) {
            should.exist(res)
            db.replicate.to('http://0.0.0.0:13370').on('error', function (res) {
              should.exist(res)
              done()
            })
          })
      ...
      it('Replication with filter that leads to some empty batches (#2689)', function (done) {
        var db = new PouchDB(dbs.name)
        var remote = new PouchDB(dbs.remote)
        var docs1 = [
          { _id: '0', integer: 0 },
      ...
      it('#3999-1 should not start over if last_seq mismatches', function () {
        ...
        return source
          ...
          .then(function () {
            ...
            return Promise.all([target.get(checkpoint), source.get(checkpoint)])
          })
          .then(function (res) {
            ...
            function (err, result) {
              ...
              db.replicate.to(
                ...
                function (err, result) {
                  ...
                  result.docs_read.should.equal(
                    1,
                    'second replication, docs_read'
                  )
                  result.docs_written.should.equal(
                    1,
                    'second replication, docs_written'
                  )
                  result.doc_write_failures.should.equal(
                    0,
                    'second replication, doc_write_failures'
                  )
                  db.info(function (err, info) {
                    verifyInfo(info, {
                      update_seq: 2,
      ...
      it('Test consecutive replications with different query_params and promises', function (done) {
        var db = new PouchDB(dbs.name)
        var remote = new PouchDB(dbs.remote)
        var myDocs = [
          { _id: '0', integer: 0, string: '0' },
      ...
      it('doc count after multiple replications', function (done) {
        ...
        function bulkLoad(db, docs, callback) {
          db.bulkDocs({ docs }, function (err, results) {
            if (err) {
              console.error(
                'Unable to bulk load docs.  Err: ' + JSON.stringify(err)
              )
              return
            }
            callback(results)
          ...
          remote
            ...
            .then(function () {
              return remote
                ...
                .then(function (revs) {
                  revs.length.should.equal(
                    2,
                    'correct number of open revisions'
                  )
                  revs[0].ok._id.should.equal(
                    docid,
                    'rev 1, correct document id'
                  )
                  revs[1].ok._id.should.equal(
                    docid,
                    'rev 2, correct document id'
                  )
          ...
          return remote
            ...
            .then(function (res) {
              ...
              res.errors.forEach(function (e) {
                e.name.should.equal(
                  testUtils.errors.FORBIDDEN.name,
        ...
        testUtils.isCouchDB(function (isCouchDB) {
          ...
          return remote
            ...
            .then(function (res) {
              ...
              res.errors.forEach(function (e) {
                e.name.should.equal(
                  testUtils.errors.UNAUTHORIZED.name,
                  'correct error name returned'
                )
              })
        ...
        testUtils.isCouchDB(function (isCouchDB) {
          ...
          return remote
            ...
            .then(function (res) {
              ...
              ids.length.should.equal(res.errors.length, 'doc ids are unique')
              return remote.allDocs({ limit: 0 })
            })
            .then(function (res) {
              res.total_rows.should.equal(2) // 1 plus the validate doc
      ...
      it('#3606 - live replication with filtered ddoc', function () {
        ...
        return remote
          ...
          .then(function () {
            return new Promise(function (resolve, reject) {
              var replicate = remote.replicate
                .to(db, {
      ...
      it('#4293 Triggers extra replication events', function (done) {
        ...
        var push = remote.replicate
          .from(db, { live: true })
          .on('paused', paused)
          .on('complete', complete)

        var pull = db.replicate
          .from(remote, { live: true })
          .on('change', change)
          .on('complete', complete)
      ...
      it('#5452 Cleanly fail with no unhandled promises on a bad connection', function (done) {
        ...
        var db = new PouchDB(dbs.name)
        var remote = new PouchDB('http://localhost:9382/does_not_exist', {
          skip_setup: true
        })
...
function assertSince(opts, expectedSince) {
  if (expectedSince !== false) {
    if (typeof opts.since === 'number') {
      ...
      throw new Error(
        "Can't handle type for opts.since: " +
          typeof opts.since +
          ' (value=' +
          opts.since +
          ')'
