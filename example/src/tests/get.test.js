  ...
  describe('test.get.js-' + adapter, function () {
        ...
        db.get(info.id, function (err, doc) {
          ...
          db.get(info.id + 'asdf', function (err) {
            ...
            err.name.should.equal(
              testUtils.errors.MISSING_DOC.name,
    ...
    it('Test opts.revs=true with rev other than winning', function (done) {
      ...
      testUtils.putBranch(db, docs, function () {
        db.get(
          ...
          function (err, doc) {
            doc._revisions.ids.length.should.equal(
              3,
              'correct revisions length'
            )
            ...
            db.put(
              ...
              function () {
                db.get(info.id, { revs_info: true }, function (err, doc) {
                  doc._revs_info.length.should.equal(
                    3,
                    'updated a doc with put'
                  )
                  done()
                })
              }
              ...
              db.put(conflicts[1], { new_edits: false }, function () {
                db.put(conflicts[2], { new_edits: false }, function () {
                  db.get(
                    '3',
                    {
                      open_revs: ['2-aaa', '5-nonexistent', '3-bbb']
                    },
                    function (err, res) {
                      res.sort(function (a, b) {
                        if (a.ok) {
        ...
        db.get(
          ...
          function (err) {
            ...
            db.get(
              'foo',
              {
                open_revs: ['1-almost', '2-correct', 'keys']
              },
              function (err) {
    ...
    it('#5883 Testing with duplicate rev hash', function (done) {
      ...
      db.bulkDocs(
        ...
        .then(function (doc) {
          doc._revisions.start.should.equal(3)
          doc._revisions.ids.should.eql([
            '0a21b4bd4399b51e144a06b126031edc',
            'updated',
            'created'
          ])
          done()
    ...
    it('5857 - open_revs with latest=true', function () {
      var db = new PouchDB(dbs.name)
      var first = null
      return db.post({ version: 'first' }).then(function (info) {
        first = info.rev
        return db
          .put({
            _id: info.id,
    ...
    it('5857 - GET losing, old revision with latest=true', function () {
      ...
      return db
        .bulkDocs(doctree, { new_edits: false })
        .then(function () {
          return db.get('mydoc', {
            rev: '2-b2',
            latest: true
